...
function MyDeveloperProperties() {
  const { user } = useAuth();
  ...
  const fetchProperties = () => {
    ...
    fetch(`http://localhost:3000/api/developer-properties/user/${user._id}`)
      .then(res => res.json())
      .then(data => {
  ...
  const handlePropertySubmit = async (e) => {
    ...
    const url = editingProperty 
      ...
      : `http://localhost:3000/api/developer-properties/user/${user._id}`;
          ...
          <div className="bg-[#1a1a1a] p-6 rounded-lg mb-6 border border-[#252525]">
            ...
            <form onSubmit={handlePropertySubmit} className="space-y-4">
              ...
              <input
                ...
                value={propertyForm.media[0]}
                onChange={(e) => setPropertyForm({ ...propertyForm, media: [e.target.value] })}
                className="w-full px-4 py-3 rounded bg-[#252525] border border-gray-700 focus:border-[#703BF7] focus:outline-none"
              />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {properties.map((prop) => (
              <div key={prop._id} className="bg-[#1a1a1a] p-6 rounded-lg border border-[#252525]">
                {prop.media && prop.media[0] && (
                  <img src={prop.media[0]} alt={prop.title} className="w-full h-48 object-cover rounded-lg mb-4" />
                )}
                <h3 className="text-xl font-bold mb-2">{prop.title}</h3>
                <p className="text-[#703BF7] font-bold mb-2">${prop.price?.toLocaleString()}</p>
                <p className="text-gray-400 text-sm mb-2">{prop.address?.city}, {prop.address?.state}</p>
                <p className="text-gray-400 text-sm mb-4">{prop.description}</p>
                <div className="flex gap-2">
                  <button
                    onClick={() => { setEditingProperty(prop); setPropertyForm(prop); setShowPropertyForm(true); }}
...
export default MyDeveloperProperties;